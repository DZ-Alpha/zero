...
- **보안 사고 대응**: 이전 SonarQube admin 비밀번호가 이미 GitHub에 push된 커밋
...
8. ~~VM 쪽 별도 보안 항목: bruce 계정 영구 비밀번호로 재변경~~ **완료(2026-07-13), 변경 불필요로 종결**
   — 팀 비밀번호 규칙(기본값 → 서비스 정책상 안 통과할 때만 승급값)을 그대로 적용하기로 결정.
   리눅스 계정은 별도 복잡도 정책이 없어 기본값이 그대로 통과하므로, 현재 설정값 자체가 이미
   팀 규칙을 준수하는 "영구값"으로 확정됨. harbor VM, monitoring VM(103),
   템플릿(harbor-template-v2, 108) 전부 동일하게 적용된 상태.
   **실제 값은 `harbor.credentials.local.md`(git 미추적)에만 기록한다.**
...
- bruce 계정 비밀번호 — 팀 규칙 기본값 자체가 이미 복잡도 요건을 충족해 별도 변경 불필요로 종결
  (실제 값은 `harbor.credentials.local.md` 참고)
