...
export function uploadDietPhoto(token: string, objectKey: string, mealType?: "BREAKFAST" | "LUNCH" | "DINNER" | "SNACK", date?: string) {
  return apiRequest<{ meal_log_id: string; status: string }>("/diet/upload", {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    body: JSON.stringify({ object_key: objectKey, ...(mealType ? { mealType } : {}), ...(date ? { date } : {}) }),
  });
}
